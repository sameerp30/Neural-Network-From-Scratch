...
def PCA_t(train_input, dev_input, test_input , num_components):
    
    train_input_pca = train_input - np.mean(train_input , axis = 0)
    dev_input_pca = dev_input - np.mean(dev_input , axis = 0)
    test_input_pca = test_input - np.mean(test_input , axis = 0)
    
    cov_matrix = np.cov(train_input_pca , rowvar = False)
    ...
    return reduced_train,reduced_dev,reduced_test
...
#     print("********************************")
    ...
    train_input,dev_input,test_input=PCA_t(train_input,dev_input,test_input,88)
    NUM_FEATS = 88
